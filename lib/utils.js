 ...
 * Strips common site-name suffixes that browsers append to document.title
 * (e.g. " - Confluence", " | My Wiki") so the folder name matches the
 * page title as the user sees it and as Web Clipper names the note.
 */
function sanitiseTitle(title) {
  return (
    title
      ...
      .replace(/(\s+-+)+$/, '')           // trailing " -" left by separator replacement (e.g. "| " → "- ")
  ) || 'Untitled';
}
